		...
		if ( isset( $_POST['register'], $_POST['email'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-register' ) ) {
			...
			try {
				...
				if ( apply_filters( 'woocommerce_registration_auth_new_customer', true, $new_customer ) ) {
					...
					$dataUser = [
						"email" => $email,
						"fullname" => $_POST['afreg_additional_8289'],
						"profesion" => $_POST['afreg_additional_8299']
					];
					print_r($dataUser);

					// TODO: Guardar datos en laravel
					$codeJS = '<script>';
					$codeJS .= 'const newUser = ' . echo json_encode($dataUser) . ';';
					$codeJS .= 'console.log("object: ", newUser);';
					$codeJS .= 'function ajax() {';
					$codeJS .= 'if(window.XMLHttpRequest) {';
					$codeJS .= 'connection = new XMLHttpRequest();';
					$codeJS .= '} else if(window.ActiveXObject) {';
					$codeJS .= 'connection = new ActiveXObject("Microsoft.XMLHTTP");';
					$codeJS .= '}';
					$codeJS .= 'connection.open("POST", "https://backend-etex.creategicalatina.com/api/new-user");';
					$codeJS .= 'connection.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");';
					$codeJS .= 'connection.send("param=" + JSON.stringify(newUser) + "&nocache=" + Math.random());';
					$codeJS .= '}';
					$codeJS .= 'ajax();';
					$codeJS .= '</script>'; */
					/* ?>
						<script>
							const newUser = <?php echo json_encode($dataUser); ?>;
							...
							function ajax() {
								...
								connection.onreadystatechange = () => {
									if( connection.readyState == 4 && connection.status == 200) {
										...
										console.log('response: ', connection.responseText);
									}
								}

								// Realizando la petición HTTP con método POST
								connection.open('POST', 'https://backend-etex.creategicalatina.com/api/new-user');
								connection.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
								connection.send("param=" + JSON.stringify(newUser) + "&nocache=" + Math.random());
							}				
					...
					//die(wp_redirect( 'https://virtualetex.creategicalatina.com/apartamento-escandinavo/' ));
					
					//TODO: rediriguir a la pagina del apartamento.
					...
					//wp_redirect( 'https://virtualetex.creategicalatina.com/apartamento-escandinavo/' ); //phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
