...
class iPanorama_List_Table_Items extends WP_List_Table {
	...
	function __construct() {
		...
		$this->url_preview_base = rtrim(parse_url(home_url(), PHP_URL_PATH ), '/') . '/ipanorama/virtualtour/';
	}
			...
			switch($item_status) {
				case 'trash': {
					...
					$actions['restore'] = sprintf(
						...
						esc_html__('Restore', IPANORAMA_PLUGIN_NAME)
					);
					...
					$actions['delete'] = sprintf(
						...
						esc_html__('Delete Permanently', IPANORAMA_PLUGIN_NAME)
					);
				} break;
				default: {
					$args = array(
					...
					$actions['edit'] = sprintf(
						'<a href="%1$s">%2$s</a>',
						esc_url($url),
						esc_html__('Edit', IPANORAMA_PLUGIN_NAME)
					);
					...
					$actions['copy'] = sprintf(
						...
						esc_html__('Duplicate', IPANORAMA_PLUGIN_NAME)
					);
					...
					$actions['trash'] = sprintf(
						...
						esc_html__('Move to Trash', IPANORAMA_PLUGIN_NAME)
					);
					...
					$actions['view'] = sprintf(
						...
						esc_url($url),
						esc_html__('Preview', IPANORAMA_PLUGIN_NAME)
					);
				} break;
			}
			
			return sprintf('<a href="?page=%1$s&id=%2$s" class="row-title">%3$s</a> %4$s',
				IPANORAMA_PLUGIN_NAME . '_item',
				$item['id'],
				$item['title'],
	...
	function column_shortcode($item) {
		return sprintf('<code>[ipano id="%1$s"]</code>', $item['id']);
	}
	...
	function get_views() {
		...
		$status_links = array(
			'all'   => sprintf('<a href="%1$s" %2$s>%3$s <span class="count">(%4$d)</span></a>', esc_url($url_all),   ($item_status            ? '' : 'class="current"'), esc_html__('All',IPANORAMA_PLUGIN_NAME), $this->count_all),
			'mine'  => sprintf('<a href="%1$s" %2$s>%3$s <span class="count">(%4$d)</span></a>', esc_url($url_mine),  ($item_status == 'mine'  ? 'class="current"' : ''), esc_html__('Mine',IPANORAMA_PLUGIN_NAME), $this->count_mine),
			'trash' => sprintf('<a href="%1$s" %2$s>%3$s <span class="count">(%4$d)</span></a>', esc_url($url_trash), ($item_status == 'trash' ? 'class="current"' : ''), esc_html__('Trash',IPANORAMA_PLUGIN_NAME), $this->count_trash)
		);
	...
	function get_bulk_actions() {
		...
		switch($item_status) {
			case 'trash': {
				$actions = array(
					'restore' => esc_html__('Restore', IPANORAMA_PLUGIN_NAME),
					'delete'  => esc_html__('Delete Permanently', IPANORAMA_PLUGIN_NAME)
				);
			} break;
			default: {
				$actions = array(
					'copy'  => esc_html__('Duplicate', IPANORAMA_PLUGIN_NAME),
					'trash' => esc_html__('Move to Trash', IPANORAMA_PLUGIN_NAME)
				);
			} break;
		}
