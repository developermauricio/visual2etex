	...
	function woodmart_my_account_wrapp_start(){ //TODO: aqui inicia la pagina de mi cuenta
		...
		if ( $afreg_extra_fields ) {
			...
			foreach ($afreg_extra_fields as $afreg_field) {
				...
				if ( $afreg_field_type == 'text' ) {
					if ( $afreg_field->ID == 8289 ) $fullName = get_user_meta( intval($user->ID), 'afreg_additional_' . intval($afreg_field->ID), true );	
					if ( $afreg_field->ID == 8602 ) $phone = get_user_meta( intval($user->ID), 'afreg_additional_' . intval($afreg_field->ID), true );	
				}
		...
		?>
			<script>
				...
				jQuery.ajax({
					url: 'https://backend.visual2etex.com/api/register-or-login',
					type: "POST",
					data: newUser,
