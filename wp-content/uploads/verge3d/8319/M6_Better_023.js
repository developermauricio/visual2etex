...
(function() {
    ...
    var sceneURL = params.load ? params.load : '__URL__M6_Better_023.gltf'.replace('__URL__', '');
    if (!sceneURL) {
...
function runCode(app) {
    /*  Elements */
	const placaAnotation = document.getElementById("lamina");
    ...
    const iconInfo = $('#content-icon-more-info');
    const contentInfo = $('#contentPopupInfo');
    const closeInfo = $('#closePopupInfo');

    iconInfo.on('click', () => { contentInfo.css('display', 'block') });
    closeInfo.on('click', () => { contentInfo.css('display', 'none') });


    const iconDocs = $('#content-icon-documents');
    const contentPopusDocs = $('#contentPopupDocuments');
    const closePopupDocs = $('#closeDocs');

    iconDocs.on('click', () => { contentPopusDocs.css('display', 'block') });
    closePopupDocs.on('click', () => { contentPopusDocs.css('display', 'none') });


    const textBrand = $('#content-text-brand');
    const contentBrand = $('#contentPopupBrand');
    let viewBrand = true;

    textBrand.on('click', () => { 
        if ( viewBrand ) {
            contentBrand.css('display', 'none') 
            viewBrand = false;
        } else {
            contentBrand.css('display', 'block') 
            viewBrand = true;
        }
    });

    const textPhotos = $('#contentTextPhotos');
    const contentPhotos = $('#contentPopupPhotos');
    let viewPhotos = true;

    textPhotos.on('click', () => { 
        if ( viewPhotos ) {
            contentPhotos.css('display', 'none') 
            viewPhotos = false;
        } else {
            contentPhotos.css('display', 'block') 
            viewPhotos = true;
        }
    });


    const textVideo = $('#contentTextVideos');
    const contentVideo = $('#contentPopupVideos');
    let viewVideo = true;
